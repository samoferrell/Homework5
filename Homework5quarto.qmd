...
prediction_test <- predict(fit1, testTransformed)
prediction_test
```

```{r}
postResample(predict(fit1, newdata = testTransformed), testTransformed$HeartDiseaseF)
```
...
```{r}
#glmFit1 <- glm(model1, data = train, family = "binomial")
#glmFit2 <- glm(model2, data = train, family = "binomial")
#glmFit3 <- glm(model3, data = train, family = "binomial")
```
...
```{r}
postResample(predict(glmFit3, newdata = test), test$HeartDiseaseF)
```
...
Model for Tree method comparison
```{r}
model <- HeartDiseaseF ~ Sex + Age + ExerciseAngina + Oldpeak + ChestPainType
```

#### classification tree model 
(use method = rpart: tuning parameter is cp, use values 0, 0.001, 0.002, . . . , 0.1)
```{r}
treeFit <- train(model, 
              data = train,
              method = "rpart",
...
#### random forest 
(use method = rf: tuning parameter is mtry, use values of 1, 2, . . . , # of predictors)
(bagging is a special case here!)
...
```
boosted is most accuracte - nearly 80%
